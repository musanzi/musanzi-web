...
export class ProjectForm {
  ...
  protected readonly projectResource = httpResource<IProject>(() =>
    this.projectId ? { url: `/projects/${this.projectId}` } : undefined
  );

  protected readonly projectModel = linkedSignal<IProjectForm>(() => {
    const project = this.projectResource.value();
    return project ? this.createFormValue(project) : { name: '', summary: '', links: [] };
  });
  protected readonly existingImageUrl = computed(() => getProjectImageUrl(this.projectResource.value()?.image ?? null));
